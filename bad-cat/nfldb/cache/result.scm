...
(define (json->game-data game-id game-json)
  (define (json->play team-id play-json)
    (let ( (quarter     (1- (json-ref period.number play-json)))
           (type-id     (if (member 'type (json-keys play-json)) (string->number (json-ref type.id play-json)) #f))
           (type-text   (if (member 'type (json-keys play-json)) (json-ref type.text play-json) #f))
...
;   (format #t "play-id: ~a, keys = ~a~%"(json-ref id play-json) (json-keys play-json)) 
    (let ( (play-id     (string->number (json-ref id play-json)))
           (type-id     (if (member 'type (json-keys play-json))
                          (string->number (json-ref type.id play-json))
                          (string->number (json-ref id play-json))))
