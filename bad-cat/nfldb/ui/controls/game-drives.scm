...
;       (format #t "Drawing from ~4a - ~8a : ~a~%" top (car part-pair) (cdr part-pair))
        (case (car part-pair)
          ( (home)
;           (format #t "Drawing home from ~a to ~a~%" (floor top) (floor (+ top (cdr part-pair))))
            (cairo-rectangle cr 0 top width (cdr part-pair))
            (cairo-set-source-rgb cr 1 0.5 0)
            (cairo-fill cr) )
          ( (away)
;           (format #t "Drawing away from ~a to ~a~%" (floor top) (floor (+ top (cdr part-pair))))
            (cairo-rectangle cr 0 top width (cdr part-pair))
            (cairo-set-source-rgb cr 0 0.5 1)
            (cairo-fill cr) )
          ( (half quarter)
;           (format #t "Drawing break from ~a to ~a~%" (floor top) (floor (+ top (cdr part-pair))))
            (cairo-rectangle cr 0 top width (cdr part-pair))
